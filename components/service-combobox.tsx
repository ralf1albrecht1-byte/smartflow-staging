...
interface ServiceComboboxProps {
  value: string; // current service name (from dropdown or manual)
  services: ServiceOption[];
  onChange: (name: string, service?: ServiceOption) => void; // called on select or type
  onServiceCreated?: (service: ServiceOption) => void; // called after saving new reusable service
  /** Current item's price — used as defaultPrice when saving a new reusable service */
  currentPrice?: string;
  /** Current item's unit — used as unit when saving a new reusable service */
  currentUnit?: string;
  /** Document type label for one-time usage hint (Auftrag/Angebot/Rechnung) */
  contextLabel?: string;
  placeholder?: string;
}
...
export function ServiceCombobox({
  ...
  // Determine if current value is a manual (non-existing) service
  const isManual = value.trim() !== '' && !services.some(s => s.name.toLowerCase() === value.trim().toLowerCase());

  // Reset justSaved when value changes or becomes manual again
  useEffect(() => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setQuery(val);
    onChange(val); // update parent with typed text (no service match)
  ...
  // Whether this is an existing/saved service (selected from list or just saved)
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div className="flex items-center gap-1.5">
        ...
        {/* Save as reusable service button — only when manual and not empty */}
        {isManual && (
          <button
            type="button"
      ...
      {/* Success indicator after saving */}
