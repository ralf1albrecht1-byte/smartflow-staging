...
if (phoneDigits && phoneDigits.length >= 6) {
  ...
  const allNotes = orders.map((o) => o.notes || '').join('\n');

  const metaLines = allNotes
    .split(/\r?\n/)
    .filter((l) => /\[META\]/i.test(l) || /Telefon\s*\(Absender/i.test(l) || /^\s*Telefon\s*:/i.test(l))
    .join('\n');
...
findings.push({
  ...
  nameStartsWithFallback: (c.name || '').trimStart().startsWith('⚠️ Unbekannt'),
...
// audit-script-repaired-2026-04-28
