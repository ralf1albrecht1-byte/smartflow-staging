...
const statusColors: Record<string, string> = {
  ...
  "In Bearbeitung":
    "bg-blue-200 text-blue-900 dark:bg-blue-900/40 dark:text-blue-200 border border-blue-300",
  Erledigt:
    "bg-green-200 text-green-900 dark:bg-green-900/40 dark:text-green-200 border border-green-300",
  Entwurf:
    "bg-gray-200 text-gray-800 dark:bg-gray-800 dark:text-gray-200 border border-gray-300",
  Gesendet:
    "bg-blue-200 text-blue-900 dark:bg-blue-900/40 dark:text-blue-200 border border-blue-300",
  Angenommen:
    "bg-green-200 text-green-900 dark:bg-green-900/40 dark:text-green-200 border border-green-300",
  Abgelehnt:
    "bg-purple-200/60 text-purple-900 dark:bg-purple-900/30 dark:text-purple-200 border border-purple-300",
  Bezahlt:
    "bg-green-200 text-green-900 dark:bg-green-900/40 dark:text-green-200 border border-green-300",
};
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="h-7 w-14 bg-muted rounded animate-pulse mt-1.5" />
          </div>
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`w-10 h-10 rounded-lg flex items-center justify-center ${review.total > 0 ? "text-orange-600 bg-orange-50 dark:bg-orange-900/20" : "text-green-600 bg-green-50 dark:bg-green-900/20"}`}
                    >
                      {review.total > 0 ? (
                    ...
                    <div className="mt-2 space-y-1 border-t pt-2">
                      {review.incompleteCustomers > 0 && (
                        ...
                        >
                          ...
                          {review.incompleteCustomers} Kundendaten unvollständig
                        </Link>
                      )}
                      {review.uncertainAssignments > 0 && (
                        ...
                        >
                          ...
                          {review.uncertainAssignments} unsichere Zuordnung
                        </Link>
                      )}
                    </div>
                  ) : (
              ...
              {loading ? (
                ...
                <div className="space-y-1">
                  {data?.recentOrders?.map((order: any) => (
                    <div
                      key={order?.id}
                      className="flex items-center justify-between p-2 rounded-lg bg-muted/50 hover:bg-muted transition-all cursor-pointer"
                      onClick={() =>
                        router.push(`/auftraege?edit=${order?.id}`)
                      }
                    >
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                        ...
                        <span className="font-mono text-xs font-bold whitespace-nowrap tabular-nums">
                          {formatCurrency(
                            Number(order?.total || order?.totalPrice || 0),
                            order?.currency === "EUR" ? "EUR" : "CHF",
                          )}
                        </span>
                      </div>
                    </div>
                ...
                <div className="space-y-1">
                  {data?.recentOffers?.map((off: any) => (
                    <div
                      key={off?.id}
                      className="flex items-center justify-between p-2 rounded-lg bg-muted/50 hover:bg-muted transition-all cursor-pointer"
                      onClick={() => router.push(`/angebote?edit=${off?.id}`)}
                    >
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-xs truncate">
                          {off?.items?.[0]?.description || off?.offerNumber}
                        </p>
                        <p className="text-[11px] text-muted-foreground truncate">
                          {off?.customer?.name || "(kein Name)"}
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          {off?.intakeTime
                            ? `Eingang: ${formatDate(off.intakeTime)}`
                            : formatDate(off?.createdAt)}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                        <Badge
                          variant="secondary"
                          className={`text-[10px] px-1.5 py-0 ${statusColors[off?.status ?? ""] ?? ""}`}
                        >
                          {off?.status ?? ""}
                        </Badge>
                        <span className="font-mono text-xs font-bold whitespace-nowrap tabular-nums">
                ...
                <div className="space-y-1">
                  {data?.recentInvoices?.map((inv: any) => (
                    <div
                      key={inv?.id}
                      className="flex items-center justify-between p-2 rounded-lg bg-muted/50 hover:bg-muted transition-all cursor-pointer"
                      onClick={() => router.push(`/rechnungen?edit=${inv?.id}`)}
                    >
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-xs truncate">
                          {inv?.items?.[0]?.description || inv?.invoiceNumber}
                        </p>
                        <p className="text-[11px] text-muted-foreground truncate">
                          {inv?.customer?.name || "(kein Name)"}
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          {inv?.intakeTime
                            ? `Eingang: ${formatDate(inv.intakeTime)}`
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                        <Badge
                          variant="secondary"
                          className={`text-[10px] px-1.5 py-0 ${statusColors[inv?.status ?? ""] ?? ""}`}
                        >
                          {inv?.status ?? ""}
                        </Badge>
                        <span className="font-mono text-xs font-bold whitespace-nowrap tabular-nums">
