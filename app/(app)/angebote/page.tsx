...
const offer_CONTACT_REVIEW_COMPACT_DATE_ONLY_V17_90L371V =
  /^\s*(?:[0-3]\d[01]\d(?:20)?\d{2}|\d{1,2}[.\/-]\d{1,2}(?:[.\/-]\d{2,4})?\.?)\s*$/;

function offer_isAppointmentOnlyContactValueV17_90L371V(value?: string | null): boolean {
  ...
  const hasContact = offer_CONTACT_REVIEW_CONTACT_LINE_V17_90L371Q.test(text) ||
    /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.test(text) ||
    (/\+?\d[\d\s().\/-]{6,}\d/.test(text) && !looksLikeCompactDate);
  const hasAppointment = offer_CONTACT_REVIEW_APPOINTMENT_LINE_V17_90L371Q.test(text);
...
function offer_stripAppointmentOnlyContactReviewTextV17_90L371Q(value?: string | null): string | null {
  ...
  if (offer_isAppointmentOnlyContactValueV17_90L371V(source)) return null;
...
function offer_contactReviewIdentityKeyV17_90L371V(record: any): string {
  const customer = record?.customer || {};
  const name = compactOfferValue(customer?.name || record?.customerName || record?.name || "").toLocaleLowerCase("de-CH");
  const phone = compactOfferValue(customer?.phone || record?.phone || "").replace(/\D+/g, "");
  const email = compactOfferValue(customer?.email || record?.email || "").toLocaleLowerCase("de-CH");
  const channel = compactOfferValue(record?.preferredChannel || record?.contactChannel || record?.channel || "").toLocaleLowerCase("de-CH");
...
const ensureOfferExecutionSiteUiKeysV17_90L302 = (
  sites: OfferExecutionSite[],
  seed: string,
): OfferExecutionSite[] => {
  const seen = new Set<string>();
  return sites.map((site, index) => {
    const rawKey = compactOfferValue(site._workSiteUiKey);
    const stableKey = rawKey && !seen.has(rawKey)
      ? rawKey
      : `offer-site-${seed || "draft"}-${index}`;
    seen.add(stableKey);
...
function collectOfferScopedAccessHintLinesV17_90L372(orders: any[]): string[] {
  ...
  for (const order of orders || []) {
    ...
    const pushScoped = (siteLabel: string, rawHint: unknown) => {
      const hint = compactOfferValue(rawHint)
    ...
    for (const line of lines) {
      ...
      if (resolved) {
        const escaped = escapeOfferWorkSiteContextRegExpV17_90L372(resolved.key);
      ...
      if (activeSiteLabel && isOfferAccessOrKeyHintLineV17_90L372(line)) {
...
function buildOfferInfoSummary(
  ...
  if (preferCanonicalWorkflowV17_90L273) {
    ...
    const scopedOfferAccessLinesV17_90L374 = collectOfferScopedAccessHintLinesV17_90L372(sourceOrders);
    const rawOfferPrimaryLinesV17_90L374 = uniqueOfferInfoLinesV17_66([
      ...canonicalWorkflowSummaryV17_90L274.primary,
      ...manualPrimary,
    ]);
    const rawOfferAdditionalLinesV17_90L374 = uniqueOfferInfoLinesV17_66([
...
function buildOfferServiceReviewSummary(
  ...
  (offer.items || []).forEach((item: any) => {
    ...
    const blockerReasons = [
      !String(item?.description || "").trim() ? "Positionsname fehlt" : "",
...
export default function AngebotePage() {
  ...
  const getSelectedCustomerExecutionAddressV17_90L296 = (site: OfferExecutionSite) => {
    const addressId = compactOfferValue(site.customerExecutionAddressId);
  ...
  const customerExecutionAddressChangedV17_90L296 = (site: OfferExecutionSite) => {
  ...
  const shouldShowCustomerExecutionAddressSaveCheckboxV17_90L298 = (_site?: OfferExecutionSite | null) => {
    // V17.90L306: Die alte Checkbox wird durch den eindeutigen Entscheidungsblock
    // ersetzt. So gibt es keine doppelte Logik zwischen Angebot und Kundenprofil.
    return false;
  };

  const shouldPersistCustomerExecutionAddressChoiceV17_90L305 = (site: OfferExecutionSite) => {
      ...
      <div className="rounded-md border border-amber-300 bg-amber-50/70 p-2.5 text-xs dark:border-amber-800 dark:bg-amber-950/20">
        ...
        <div className="flex flex-col gap-2">
          <label className="flex items-start gap-2">
            ...
            />
            <span>
              <span className="font-medium">Nur in diesem Angebot übernehmen</span>
  ...
  const addExecutionSite = () => {
    ...
    setItems((current: OfferItem[]) => {
      // V17.90L293: Beim ersten Arbeitsort bleiben vorhandene Positionen
...
Die Löschung wird erst mit „Speichern“ dauerhaft übernommen.`,
  ...
  const openEditOffer = (
    ...
    const offerExecutionSites = ensureOfferExecutionSiteUiKeysV17_90L302(
    ...
    if (off.items && off.items.length > 0) {
      setItems(
        off.items.map((i: any) => {
          const matchedExecutionSiteV17_90L302 =
            offerExecutionSites.find((site) =>
              compactOfferValue(site._workSiteUiKey) &&
          ...
          return {
            ...
            siteName: i.siteName || matchedExecutionSiteV17_90L302?.siteName || null,
            siteAddress:
              i.siteAddress || matchedExecutionSiteV17_90L302?.siteAddress || null,
            sitePlz: i.sitePlz || matchedExecutionSiteV17_90L302?.sitePlz || null,
            siteCity: i.siteCity || matchedExecutionSiteV17_90L302?.siteCity || null,
            siteNote: i.siteNote || matchedExecutionSiteV17_90L302?.siteNote || null,
            sourceOrderId:
              i.sourceOrderId || matchedExecutionSiteV17_90L302?.sourceOrderId || null,
            _workSiteUiKey: matchedExecutionSiteV17_90L302?._workSiteUiKey || null,
          };
        }),
      );
    } else {
  ...
  const saveOffer = async (
    itemsOverride?: OfferItem[],
    executionSitesOverride?: OfferExecutionSite[],
    forceClearExecutionAddressV17_90L302 = false,
  ): Promise<any | null> => {
    const sourceItems = itemsOverride ?? items;
    const sourceExecutionSites = executionSitesOverride ?? executionSites;
    if (!form?.customerId) {
      toast.error("Bitte Kunde wählen");
      return null;
    }
    if (!sourceItems?.some((item) => compactOfferValue(item.description))) {
    ...
    const itemsForSave = applyExecutionSitesToOfferItems(sourceItems, sourceExecutionSites);
    const completeExecutionSitesV17_90L292 = sourceExecutionSites.filter(
      (site) =>
        Boolean(compactOfferValue(site.siteAddress)) &&
        Boolean(compactOfferValue(site.sitePlz)) &&
        Boolean(compactOfferValue(site.siteCity)),
  ...
  const persistAndAcceptOfferExecutionSiteV17_90L295 = async (
    ...
    setSaving(true);
    try {
      let executionSitesForSaveV17_90L302 = executionSites;
      ...
      if (shouldPersistCustomerExecutionAddressV17_90L305) {
        ...
        executionSitesForSaveV17_90L302 = executionSites.map((entry) =>
        ...
        setExecutionSites(executionSitesForSaveV17_90L302);
      }

      const saved = await saveOffer(undefined, executionSitesForSaveV17_90L302);
      if (!saved) return;
      if (!editOfferId && saved?.id) setEditOfferId(saved.id);
      setFromOrderId(null);
      setExecutionAddressEditSnapshot(
  ...
  const getOfferToInvoiceBlockersV17_90L174 = (
    ...
    if (!entries.length) blockers.push("Keine Positionen vorhanden");
  ...
  const saveAndCreateInvoice = async () => {
    ...
    try {
      ...
      const invoiceItemsForHandoffV17_90L371CC = applyExecutionSitesToOfferItems(
        offerItems,
        executionSites,
      );
      const conversionBlockers = getOfferToInvoiceBlockersV17_90L174(
        invoiceItemsForHandoffV17_90L371CC,
      );
      ...
      const invRes = await fetch("/api/invoices", {
        ...
        body: JSON.stringify({
          ...
          items: invoiceItemsForHandoffV17_90L371CC,
  ...
  const renderActiveMobileTooltipSheet = () => {
    ...
    const sheetTitle =
      activeMobileTooltip.reviewTitle || "Positionen prüfen";
                                ...
                                <div className="relative grid min-w-0 grid-cols-1 items-start gap-x-3 gap-y-1 sm:grid-cols-[minmax(0,1fr)_auto]">
                                  <div className="min-w-0">
                                    ...
                                    <div className="mt-1 text-[10px] font-medium text-muted-foreground sm:text-[11px]">
                                      Positionen · {mobileOfferServiceNames.length}
                                    </div>
                              ...
                              <div className="hidden min-w-0 items-stretch gap-3">
                                <div className="min-w-0 flex-1 overflow-visible">
                                  ...
                                  <div className="mt-0.5 text-[10px] font-semibold text-muted-foreground">
                                    {mobileOfferServiceNames.length} Positionen
                                  </div>
                                  <p
                      ...
                      (() => {
                        ...
                        return (
                          <div
                            ...
                            className={`rounded-xl border border-emerald-300 bg-emerald-50/70 p-2 sm:p-3 space-y-1.5 min-w-0 transition-colors dark:border-emerald-800 dark:bg-emerald-950/20 ${
                              historicalOfferCustomerLocked
                                ? "cursor-default"
                                : "cursor-pointer hover:bg-emerald-100/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/60"
                            }`}
                          >
                      ...
                      <>
                        ...
                        {form.customerId &&
                          (() => {
                            ...
                            const reqMiss = isRequiredCustomerFieldMissing;
                            return (
                              <div className="mt-2 rounded-xl border border-emerald-300 bg-emerald-50/70 p-2 sm:p-3 space-y-1.5 min-w-0 dark:border-emerald-800 dark:bg-emerald-950/20">
                                {isFallbackCustomerName(cust.name) ? (
                                  <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                                    <span className="text-sm font-semibold truncate text-amber-600 dark:text-amber-400">
                                      ⚠️ Kunde noch nicht zugeordnet
                              ...
                              >
                                ...
                                <div className="mt-0.5 grid min-w-0 grid-cols-1 items-center gap-x-8 gap-y-1 sm:grid-cols-[17rem_auto]">
                                  <div className="truncate text-xs text-muted-foreground">
                                    {positionTypeLabelV17_90L371K} · {Number(item?.quantity ?? 0) > 0
                                      ? item.quantity
                                      : "prüfen"}{" "}
                                          ...
                                          {executionSites.map((siteOption, siteIndex) => {
                                            ...
                                            return (
                                              <button
                                                ...
                                                onClick={(event) => {
                                                  event.stopPropagation();
                                                  assignOfferItemToSite(idx, siteKey);
                                                }}
                                              >
                                                <MapPin className="h-4 w-4" />
                            ...
                            >
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2 text-sm font-semibold leading-tight">
                                  ...
                                  <span>
                                    📍 {visibleOfferWorkSiteNumberV17_90L371CC}. {group.site?.siteName || group.site?.siteAddress || "Ausführungsort"}
                                  </span>
                                  <span className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-medium text-slate-700 ring-1 ring-slate-200">
                                    {group.entries.length} Position{group.entries.length === 1 ? "" : "en"}
                                  </span>
                                      ...
                                      {groupSummary.reviewCount > 0 && (
                                        ...
                                        >
                                          <span className="truncate whitespace-nowrap">
                                            Positionen prüfen · {groupSummary.reviewCount}
                                          </span>
                                          <OfferServiceReviewTooltip
                                            title={`Positionen prüfen · ${groupSummary.reviewCount}`}
                                  ...
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <div className="flex flex-wrap items-center justify-end gap-2">
                                      <Button
                                        ...
                                        onClick={() => void removeOfferExecutionSite(group.key)}
                                      >
                                        Löschen
                                      </Button>
                                  ...
                                  >
                                    ...
                                    <div className="divide-y divide-slate-200 px-2 text-sm dark:divide-slate-700">
                                      {group.entries.map(({ item, index }) => {
                                        ...
                                        return (
                                          <div
                                            key={`offer-overview-${group.key}-${index}`}
                                            className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 py-1.5"
                                          >
                                            <div className="min-w-0">
                                              <div className="font-medium">
                                                {compactOfferValue(item?.description) || "Unbenannte Position"}
                                              </div>
                                              <div className="text-xs text-muted-foreground">
                                                {quantity > 0
                                                  ? `${quantity} ${compactOfferValue(item?.unit) || "–"}`
                                                  : "Menge prüfen"}
                                                {" · "}
                                                {unitPrice > 0
                                            ...
                                            <div className="text-right font-mono text-sm font-medium">
                                              {formatCurrency(Number.isFinite(lineTotal) ? lineTotal : 0, currency)}
                                            </div>
                                          </div>
                                        );
                                      })}
