  ...
  try {
    ...
    if (imageBase64 && action === "analyze") {
      ...
      if (rawBytes > MAX_IMAGE_INPUT_BYTES) {
        ...
        return NextResponse.json(
          {
            error:
              "⚠️ Bild zu groß (max. 15 MB). Bitte kleineres Bild verwenden.",
            blocked: true,
    ...
    if (imageBase64 && action === "analyze") {
      try {
        ...
        const { optimizeImageBufferForAi } =
          await import("@/lib/image-optimizer");
        ...
        console.error(
          "[quick-intake] image optimization error, using original:",
          err?.message || err,
        );
      }
    }
    ...
    if (action === "analyze") {
      ...
      const intakeCurrency = settings?.currency === "EUR" ? "EUR" : "CHF";
      const firmenname = settings?.firmenname || "Business Manager";
      const serviceList = services
        .map(
          (s: any) =>
            `- ${s.name} (${s.defaultPrice} ${intakeCurrency}/${s.unit})`,
        )
        .join("\n");
...
Respond with raw JSON only.
{
  ...
  "matchConfidence": "strong oder weak oder null"
      ...
      if (aiImageBase64) {
        userContent.push({
          type: "text",
          text: "Der Kunde hat auch dieses Bild geschickt. Analysiere es zusammen mit den anderen Eingaben:",
        });
        userContent.push({
          type: "image_url",
          image_url: { url: `data:${aiImageMimeType};base64,${aiImageBase64}` },
        });
      }
      ...
      const unitPrice = matchedService
        ? Number(matchedService.defaultPrice)
        : 50;
      ...
      if (matchedService && parsed.unit && matchedService.unit) {
        ...
        const expectedUnit = (matchedService.unit as string)
          .trim()
          .toLowerCase();
        if (detectedUnit !== expectedUnit) {
          const sName =
            matchedService.name || parsed.serviceName || "Sonstiges";
          unitMismatchReasons.push(
            `unit_mismatch:${sName}:${parsed.unit}:${matchedService.unit}`,
          );
        }
      }
      ...
      if (parsed.existingCustomerId) {
        ...
        console.log(
          `[quick-intake] Match verification: verdict=${matchResult.verdict}, reason=${matchResult.reason}, candidateId=${parsed.existingCustomerId}`,
        );
      }
      ...
      return NextResponse.json({
        ...
        analysis: {
          ...
          matchReason,
          suggestedCustomerId:
            matchVerdict !== "kein_treffer" ? parsed.existingCustomerId : null,
          unitPrice,
          unit,
    ...
    if (action === "create") {
      ...
      if (customerId) {
        ...
        if (!existing) {
          ...
          if (matchResult.verdict !== "auto_assign") {
            ...
            console.log(
              `[quick-intake] CREATE: Rejecting auto-assignment (verdict=${matchResult.verdict}, reason=${matchResult.reason}) — user did not confirm`,
            );
            customerId = null;
          } else {
            console.log(
              `[quick-intake] CREATE: Auto-assignment verified (${matchResult.reason})`,
            );
          }
        } else {
          console.log(
            `[quick-intake] CREATE: User explicitly confirmed match for ${customerId}`,
          );
        }
      }
      ...
      if (!customerId) {
        ...
        if (exact.match) {
          ...
          console.log(
            `[quick-intake] 🎯 EXACT REUSE → binding to existing ${exact.match.customerNumber} (${exact.match.id})`,
          );
          logAuditAsync({
            userId,
          ...
          console.log(
            `[quick-intake] exact-reuse skipped (${exact.reason}, count=${exact.candidateCount}) → normal create path`,
          );
        }
      }
      ...
      if (!customerId) {
        ...
        if (nearExact.match && nearExact.completedField) {
          ...
          console.log(
            `[quick-intake] 🎯 NEAR-EXACT REUSE → binding to existing ${nearExact.match.customerNumber} (${nearExact.match.id}), completed=${nearExact.completedField}`,
          );
          logAuditAsync({
            userId,
          ...
          console.log(
            `[quick-intake] near-exact-reuse skipped (${nearExact.reason}, count=${nearExact.candidateCount}) → normal create path`,
          );
        }
      }
      ...
      if (!customerId) {
        ...
        if (sanitized.dropped.length > 0) {
          console.log(
            `[quick-intake] 🛡️ intake-sanitize dropped unverified fields on new-customer create: ${sanitized.dropped.join(", ")}`,
          );
        }

        const name =
          analysis.customerName && analysis.customerName !== "Unbekannt"
            ? analysis.customerName
            : "Neuer Kunde";
      ...
      if (
        analysis.unitMismatchReasons &&
        Array.isArray(analysis.unitMismatchReasons)
      ) {
        ...
        const createMatchedService = await prisma.service.findFirst({
          where: {
            userId,
            name: { equals: analysis.serviceName, mode: "insensitive" as any },
          },
        });
        if (
          createMatchedService &&
          analysis.unit &&
          createMatchedService.unit
        ) {
          const det = (analysis.unit as string).trim().toLowerCase();
          const exp = (createMatchedService.unit as string)
            .trim()
            .toLowerCase();
          if (det !== exp) {
            createUnitMismatchReasons.push(
              `unit_mismatch:${createMatchedService.name}:${analysis.unit}:${createMatchedService.unit}`,
            );
          }
        }
      }
      ...
      const order = await prisma.order.create({
        data: {
          ...
          hinweisLevel:
            createUnitMismatchReasons.length > 0
              ? "warning"
              : analysis.hinweisLevel || "none",
