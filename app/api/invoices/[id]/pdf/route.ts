...
async function renderPdfFromHtml(html: string): Promise<Buffer> {
  ...
  try {
    ...
    console.log(
      "[PDF DEBUG] renderPdfFromHtml called, HTML length:",
      html.length,
    );
...
export async function GET(
  ...
  try {
    ...
    if (invoice.status === "Erledigt") {
      try {
        ...
        return NextResponse.json(
          {
            error:
              "Archiviertes PDF konnte nicht geladen oder erstellt werden. Bitte versuchen Sie es später erneut.",
          },
    ...
    if (
      invoiceCompanySettings?.letterheadVisible === true &&
      invoiceCompanySettings?.letterheadUrl
    ) {
